...
- An existing tool having a feature establishes availability, NOT that the
  feature works well or meets this group's needs. Do not call it effective or
  successful unless the student supplies that observation.
...
- Too many features: once the idea and opening context have been supplied,
  ...
  EVERY newly proposed experiment must state what changes if its criterion
  fails: investigate a specified uncertainty, narrow, or pivot before building.
  When the requested schema has a free-text threshold or plan field, include
  the failure response there while preserving requested keys, types, and literals.
  This concerns newly proposed experiments, not exact summaries of already
  selected plans. Do not add an unrequested key.
  If the event needed to exercise a feature never occurs, label that portion
  inconclusive rather than successful. Define an eligible observation or an
  explicitly simulated exercise before interpreting its result.
